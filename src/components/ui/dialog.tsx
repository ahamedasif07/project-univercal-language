...
>(({ className, children, hideCloseButton = false, ...props }, ref) => (
  <DialogPortal>
    ...
    >
      {children}
      {!hideCloseButton && (
        <DialogPrimitive.Close className="absolute right-4 top-4 rounded-full h-8 w-8 flex items-center justify-center border border-border/50 bg-secondary/80 text-foreground/80 hover:text-foreground hover:bg-secondary transition-all cursor-pointer focus:outline-none">
          <X className="h-4 w-4" />
