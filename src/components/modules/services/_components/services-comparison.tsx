...
// true = yes, false = no, "partial" = partial/optional
...
function Cell({ val }: { val: boolean | "partial" }) {
  ...
  if (val === "partial")
    return <HelpCircle className="w-4 h-4 text-amber-500 mx-auto" />;
  return <X className="w-4 h-4 text-muted-foreground/40 mx-auto" />;
}

export function ServicesComparison() {
  return (
    <section className="py-20 sm:py-28 border-t border-border/40 bg-gradient-to-b from-muted/20 to-background overflow-x-auto">
      <Container>
        {/* Header */}
                  ...
                  {PACKAGES.map((pkg) => (
                    <th key={pkg.name} className="p-4 sm:p-5 text-center">
                      <div className="flex flex-col items-center gap-1.5">
                        <span
                          className={`w-8 h-8 rounded-lg bg-gradient-to-br ${pkg.gradient} flex items-center justify-center text-white text-[11px] font-black`}
                        >
                          {pkg.short.slice(0, 2)}
                        </span>
                        <span className="text-[11px] font-bold text-foreground/80 leading-tight text-center">
                          {pkg.name}
                        </span>
                        {pkg.highlight && (
