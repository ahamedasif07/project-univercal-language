        ...
        <div className="mt-16 rounded-3xl bg-gradient-to-r from-[#0b2545] via-[#0b3a82] to-[#124285] text-white p-7 sm:p-10 shadow-xl border border-blue-800/40">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-6">
            ...
            <div className="flex flex-col sm:flex-row items-center gap-3 shrink-0 w-full sm:w-auto">
              <a
                href="https://wa.me/8801674166241?text=Hello%20Universal%20Language,%20I%20want%20a%20free%20PTE%20level%20assessment%20and%20course%20guideline."
                target="_blank"
...
function HumanizedCourseCard({ item }: { item: CoursePackage }) {
  ...
  const whatsappLink = `https://wa.me/8801674166241?text=Hello%20Universal%20Language,%20I%20am%20interested%20in%20${encodeURIComponent(
    item.title
  ...
  return (
    <div
      className={cn(
        "group relative flex flex-col justify-between rounded-3xl border transition-all duration-300 overflow-hidden bg-white dark:bg-slate-900 shadow-sm hover:shadow-2xl hover:-translate-y-1.5",
        isMostPopular
          ? "border-primary/80 dark:border-blue-500 ring-2 ring-primary/20 shadow-primary/5"
          : "border-slate-200/80 dark:border-slate-800 hover:border-primary/60 dark:hover:border-blue-500/60"
      )}
    >
      <div>
        {/* Real Authentic Photo Header with Gradient Overlay & Badges */}
        <div className="relative h-52 sm:h-56 w-full overflow-hidden bg-slate-100 dark:bg-slate-800">
          ...
          <div className="absolute inset-0 bg-gradient-to-t from-[#0b2545]/90 via-[#0b2545]/40 to-black/20" />

          {/* Top Badges */}
          <div className="absolute top-3.5 left-3.5 right-3.5 flex items-center justify-between gap-2">
            <span className="px-3 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase bg-black/60 backdrop-blur-md border border-white/20 text-white shadow-xs">
              {item.format}
            </span>
            ...
            {item.badge && (
              <span
                className={cn(
                  "px-3 py-1 rounded-full text-[11px] font-bold tracking-wide uppercase shadow-sm flex items-center gap-1 text-white",
        ...
        <div className="p-5 sm:p-6 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-2 text-xs">
            <div className="p-2.5 rounded-xl bg-slate-50 dark:bg-slate-850 border border-slate-200/60 dark:border-slate-750 flex items-center gap-2">
          ...
          <div className="space-y-1.5 pt-1 border-t border-slate-100 dark:border-slate-800">
            {item.overview.features.slice(0, 3).map((feat, i) => (
              <div
                key={i}
          ...
          {/* Practice Portal Tiers Preview */}
          {item.pricingOptions && (
            <div className="space-y-1 bg-blue-50/60 dark:bg-blue-950/20 p-2.5 rounded-xl border border-blue-200/60 dark:border-blue-800/40 text-[11px] text-blue-900 dark:text-blue-300 font-semibold">
      ...
      <div className="p-5 sm:p-6 pt-0 space-y-3.5 border-t border-slate-100 dark:border-slate-800/80">
        {/* Pricing Display */}
        <div className="flex items-baseline justify-between pt-3">
          ...
          <span className="text-[11px] font-semibold text-primary dark:text-blue-300 bg-blue-50 dark:bg-blue-950/60 px-2 py-0.5 rounded-md border border-blue-200/60 dark:border-blue-800/60">
            {isOfficialService ? "Official Fee" : "All-Inclusive"}
          </span>
        </div>
        ...
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="flex-1 inline-flex items-center justify-center gap-2 py-2.5 px-4 rounded-full bg-primary hover:bg-primary/90 text-white font-bold text-xs tracking-wide transition-all duration-200 shadow-md hover:shadow-lg active:scale-98 group/btn cursor-pointer"
          >
          ...
          <a
            ...
            className="p-2.5 rounded-full border border-emerald-500/30 bg-emerald-50/50 hover:bg-[#25D366] text-[#25D366] hover:text-white dark:bg-emerald-950/30 dark:hover:bg-[#25D366] dark:hover:text-white transition-all cursor-pointer shrink-0 shadow-2xs"
          >
            <WhatsAppIcon className="w-4 h-4" />
          </a>
        </div>
